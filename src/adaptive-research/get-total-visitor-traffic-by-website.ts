...
export const getTotalVisitorTrafficByWebsiteId = async (
  ...
  if (period === "last24h") {
    ...
    currentQuery = `
      SELECT toStartOfHour(created_at) as hour, COUNT(DISTINCT visitor_id) as total
      FROM event
    ...
    currentQuery = `
      SELECT toDate(created_at) as day, COUNT(DISTINCT visitor_id) as total
      FROM event
      WHERE website_id = '${websiteId}'
        AND created_at >= '${startStart}'
        AND created_at <= '${startEnd}'
    ...
    currentQuery = `
      SELECT toYYYYMM(created_at) as month, COUNT(DISTINCT visitor_id) as total
      FROM event
      WHERE website_id = '${websiteId}'
        AND created_at >= '${startStart}'
        AND created_at <= '${startEnd}'
