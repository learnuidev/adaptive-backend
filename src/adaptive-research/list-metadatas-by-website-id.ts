...
type ListMetadatasByWebsiteIdResponse = {
  key: string;
  values: string[];
}[];

export async function listMetadatasByWebsiteId({
  clickHouseClient,
  websiteId,
}: {
  clickHouseClient: ClickHouseClient;
  websiteId: string;
}): Promise<ListMetadatasByWebsiteIdResponse> {
  const query = `
    SELECT
      key,
  ...
  const rows = (await result.json()) as ListMetadatasByWebsiteIdResponse;
  return rows;
}
